...
import { ADMIN_ONLY, CHECKIN_ROLES, EVENT_CREATE_ROLES, EVENT_STAFF_ROLES, forbiddenResponse } from "./server/mysql/routes/routeAuth";
...
export function registerMysqlApi(app: express.Express, options: MysqlApiOptions) {
  ...
  const requireEventStaff = (req: express.Request, res: express.Response) => requireRole(req, res, EVENT_STAFF_ROLES);
  ...
  registerEventStaffRoutes(app, { prefix: MYSQL_PREFIX, requireEventStaff, requireAuthorizedRead });
