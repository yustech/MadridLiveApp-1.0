...
- **Esquema real = SOLO 4 tablas (`staff`, `events`, `shifts`, `alerts`):** las crea automáticamente `initSchema()` en `mysqlApi.ts` al arrancar. NO existe ni debe existir una tabla `supervisors` — es un residuo de un diseño de auth abandonado que llegó a producción por error (alguien copió el SQL de ejemplo de `DatabaseManagerScreen.tsx`) y se eliminó el 2026-07-12 junto con una vista huérfana `STAFF COMPLETO`. La autenticación de admin NO usa base de datos: es por `ADMIN_LOGIN_EMAIL`/`ADMIN_LOGIN_PASSWORD` del `.env` con cookies firmadas en `server.ts`. No recrees `supervisors` ni añadas tablas/vistas fuera de esas 4 sin una razón real y documentada. Backups de los objetos eliminados: `/opt/madridlive-app/backups/pre-drop-*` (también en Google Drive).
