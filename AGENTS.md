...
- **Binding de red (CRÍTICO, incidente 2026-07-12):** `server.ts` usa `HOST = process.env.HOST || "0.0.0.0"`. El `.env` de producción **debe** definir `HOST=127.0.0.1` explícitamente — si falta, el backend queda expuesto sin TLS en la IP pública del puerto 3000, saltándose nginx por completo (incluye `/api/auth/login`). Nunca elimines esa línea de `/opt/madridlive-app/.env` ni de `.env.example`, y no cambies el default de `server.ts` a `0.0.0.0`. Detalle completo en `docs/PRODUCTION_OBSERVABILITY.md` → "Deploy Incident Closure 2026-07-12".
