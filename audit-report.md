...
>
> **Estado al 2026-07-26 (casillas reconciliadas contra el código, no contra la memoria): 35 tareas hechas, 1 pendiente (#32), 2 descartadas por el owner (#1 y #2).**
...
>
> **Lo siguiente no es código**: pasar el **ensayo general** (`docs/ENSAYO_GENERAL.md`) con personas y teléfonos reales, que es el único riesgo que no se puede acotar leyendo el repo. La tarea #32 va después del ensayo y antes del primer concierto real.
...
- [ ] **32. Copia offsite cifrada de los `.env` de prod y staging.** *(añadida 2026-07-26, tras el segundo drill de restauración)*
  **Contexto verificado**: la base de datos tiene copia diaria fuera de la máquina (Drive) y está probada restaurable — dos drills, el del 2026-07-23 desde Drive y el del 2026-07-26 en local (`docs/DR_RESTORE_RUNBOOK.md`). Los `.env`, en cambio, **no tienen ninguna copia offsite**: se excluyen del sync a propósito desde la PR #35 porque contienen secretos en claro, y los snapshots antiguos que había en Drive se borraron por ese mismo motivo. Consecuencia hoy: ante la pérdida total de la caja, la base de datos se recupera pero **las credenciales dependen enteramente de que el gestor de contraseñas del owner esté completo y al día** — credenciales MySQL, `ADMIN_API_TOKEN`, `ADMIN_SESSION_SECRET`, bloque `MAIL_*`, `WATCHDOG_MIN_STAFF_COUNT`. Es el último hueco real de la cadena de recuperación.
  **Cuándo**: **antes del primer concierto real, no antes del ensayo general.** Decisión razonada del 2026-07-26: el ensayo no toca los `.env` ni aumenta el riesgo de perder la máquina, y una caída durante un ensayo es gratis; en cambio, meter un cambio de infraestructura en la misma ventana que el ensayo mezcla dos estrenos y complica depurar si algo falla. Orden acordado: ensayo → arreglar lo que destape → esta tarea → concierto real.
  **Modelo/Effort**: bajo-medio, ~2 h con verificación. Puede hacerlo Claude directamente.
  **Alcance**:
  - Cifrado **simétrico** con `age` (binario único, sin infraestructura de claves) o `gpg --symmetric`; la clave/frase vive **solo** en el gestor de contraseñas del owner, nunca en la caja ni en el repo.
  - Enganchar el cifrado a `scripts/backup-sync-gdrive.sh`, que ya sube a Drive, de forma que suba `env-<entorno>-<ts>.age` junto a los dumps. **Nunca subir el `.env` en claro**: es exactamente lo que se retiró en su día.
  - Documentar el **descifrado** en `docs/DR_RESTORE_RUNBOOK.md`, en la sección de recuperación completa de la caja, sustituyendo la nota actual de "alternativa futura".
  - Verificar de verdad: cifrar, subir, descargar desde Drive en otra ruta, descifrar y comparar con el original (`diff`), y borrar la copia descifrada.
  - Ojo con los permisos: los `.env` son `600`; el fichero cifrado y cualquier temporal deben conservar `umask 077`, como ya hace `scripts/backup-mysql.sh`.
  **Prompt**: pendiente de redactar.
